...
//--------trying unpack
/* t_vec3 un_pack(t_position pos, t_img *img)
{
	t_norm_color pack;
	t_vec3		bump;

	pack = pixel_color_get(pos.i, pos.j, img);
	bump = vec(-(pack.r / 127.5 - 1), -(pack.b / 127.5 - 1), 1 - pack.g / 255, 0);
	return (bump);
} */
...
void	bump_sp(t_point obj_pnt, t_sphere sphere, t_comps *comps)
{
	...
	//bumpv = un_pack(comps->pos, sphere.texture->bump_map);
