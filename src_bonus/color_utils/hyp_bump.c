...
t_vec3 bump_h(t_position pos, t_img *img, double strength)
{
	...
	dfdx = strength * ((double)(pixel_gray_get_h((pos.i + 1), pos.j, img) - curr)) / 255.0;
	dfdy = strength * ((double)(pixel_gray_get_h(pos.i, (pos.j + 1), img) - curr)) / 255.0;
...
static inline void	hy_bump_norm(t_vec3 bumpv, t_vec3 bumpp, t_hyperboloid hyp, t_comps *comps)
{
	bumpv.w = 0;
