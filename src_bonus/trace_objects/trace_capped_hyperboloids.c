...
bool	intersect_caps_hy(t_ray ray, double *t3, double *t4)
{
	double	t;
	bool	hit1;
	bool	hit2;
	...
	*t4 = INFINITY;
	...
	hit1 = check_cap_hy(ray, t);
	if (hit1)
		*t4 = t;
	t = (1 - ray.origin.y) / ray.dir.y;
	hit2 = check_cap_hy(ray, t);
	if (hit2)
		*t3 = t;
	if (hit1 || hit2)
		return (true);
...
void	ray_hyperboloid_intersect(t_hyperboloid *hyperboloids, t_ray ray, t_intersects *intersects)
{
	...
	if (intersect_caps_hy(ray, &t3, &t4))
	{
		if (t3 < INFINITY)
