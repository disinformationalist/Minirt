...
static inline void	set_view_topleft(t_trace *trace, t_vec3 view_center, double view_width, double view_height)
{
	...
	else if (dot_product(trace->cam->orient, vec(0, 0, 1, 0)) < 0 )//&& dot_product(true_up, vec(0, 1, 0, 0)) < 0)//this works for vert but not horiz. 
	{
		right = norm_vec(cross_prod(trace->cam->orient, vec(0, 1, 0, 0)));
