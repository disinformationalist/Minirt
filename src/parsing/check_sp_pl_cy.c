...
void	check_sp(char **line, char ***rt_file)
{
	...
	if (check_double(&diam_str, -10000.0, 10000.0))
		free_exit(rt_file, "Error\n Invalid sphere diameter\n", \
	"Sphere diameter must be between 0.0 and 1.0\n");
...
void	check_cy(char **line, char ***rt_file)
{
	...
	"Cylinder diameter must be between -10000 and 10000\n");
	if (check_double(&height_str, -10000.0, 10000.0))
		free_exit(rt_file, "Error\n Invalid cylinder height\n", \
	"Cylinder height must be between -10000 and 10000\n");
