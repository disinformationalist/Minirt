	...
	t_vec3 right, t_vec3 true_up)
{
	...
	trace->move = move;
	trace->view_topleft = view_topleft;
	trace->n2 = trace->n * trace->n;
	if (trace->supersample)//needed to set the right start inside of pixel later.
	{
	...
	double view_width, double view_height)
{
	...
	if (trace->cam->has_true_up)//we have two vecs and all we need, otherwise handle special cases possible with one vec
	{
		trace->cam->true_up = norm_vec(trace->cam->true_up);
...
void	init_viewing(t_trace *trace)
{
	...
	view_height = view_width / trace->aspect_r;
