...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/16 14:08:09 by jhotchki          #+#    #+#             */
/*   Updated: 2024/12/16 14:08:11 by jhotchki         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_pl_transforms(t_trace *trace)
{
	...
	if (trace->planes)
	{
		...
		while (true)
		{
			...
			curr_pl->norm = norm_vec(mat_vec_mult(transpose(curr_pl->transform),
						vec(0, 1, 0, 0)));
...
void	set_cy_transforms(t_trace *trace)
{
	...
	if (trace->cylinders)
	{
		...
		while (true)
		{
			inv_trans = translation(-curr_cy->center.x, -curr_cy->center.y,
					-curr_cy->center.z);
