...
/* bool	ray_sphere_intersect(t_sphere sphere, t_vec3 ray.dir, t_vec3 ray.origin, double *t)
{
...
bool	ray_sphere_intersect(t_sphere sphere, t_ray ray, double *t)
{
	...
	//matrix transforming... working
	ray = transform(ray, sphere.transform);//works
...
double spotlight(t_vec3 light_dir)
{
	...
	ln = norm_vec(vec(0, 1, 0, 0));//invert orientation 
...
unsigned int color_sphere(t_trace *trace, t_ray r, t_track_hits *closest)
{
	...
	{
		int_pnt = add_vec(r.origin, scale_vec(closest->t, r.dir));
		norm = normal_at(int_pnt, sphere->transform);
